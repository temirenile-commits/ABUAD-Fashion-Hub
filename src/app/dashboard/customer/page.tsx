  ...
  return (
    <div className={`container ${styles.page}`}>
      ...
      <div className={styles.dashboardGrid}>
        {/* Main: Orders */}
        <div className={styles.mainCol} id="tour-customer-orders" data-miles-tour="customer-orders">
              ...
              {orders.map((order) => (
                <div key={order.id} className={`card ${styles.orderCard}`}>
                  <div className={styles.orderHeader}>
                    ...
                    <div className={`${styles.statusBadge} ${order.payment_system === 'manual' && order.status === 'pending' ? styles.pending_verification : styles[order.status]}`} style={order.payment_system === 'manual' && order.status === 'pending' ? { background: '#121214', color: '#FFFFFF', border: '1px solid #27272A' } : undefined}>
                  ...
                  {order.payment_system === 'manual' && order.status === 'pending' && (
                    <div style={{ margin: '0 1.5rem 1.rem', background: '#121214', border: '1px dashed #A0A0A0', padding: '1rem', borderRadius: '12px', marginBottom: '1rem' }}>
