...
export default function VendorDashboard() {
  ...
  useEffect(() => {
    try {
      const savedSide = window.localStorage.getItem('mastercart-miles-bubble-side-delicacies');
      if (savedSide === 'left' || savedSide === 'right') setMilesBubbleSide(savedSide);
    } catch {}
  }, []);
  useEffect(() => {
    try { window.localStorage.setItem('mastercart-miles-bubble-side-delicacies', milesBubbleSide); } catch {}
  }, [milesBubbleSide]);
      ...
      if (editingProduct) {
        ...
        if (!error) {
          ...
          if (isChef && !newProduct.isDraft && !isTrialActive && !isSubActive) {
            const { data: { session } } = await supabase.auth.getSession();
            if (session) {
              const creditsResponse = await fetch('/api/products/decrement-credits', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
              ...
              if (creditsResponse.ok) {
                ...
                console.error('Listing credit decrement failed', await creditsResponse.text());
              }
            }
          }
    ...
    <div className={`container ${styles.page} pb-mobile-nav`}>
        ...
        <>
          <aside className={`${styles.sidebar} ${isChef ? styles.chefSidebar : ''}`}>
              ...
              <div className={styles.brandInfo} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1.25rem' }}>
                ...
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <span style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--primary)', letterSpacing: '0.05em' }}>MASTERCART DELICACIES</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <nav className={styles.nav}>
              ...
              <div className={styles.navDivider} style={{ height: '1px', background: '#121214', marginBottom: '1rem' }} />
              ...
              <button className={`${styles.navItem} ${activeTab === 'ai' ? (isChef ? styles.navActiveChef : styles.navActive) : ''}`} onClick={() => setActiveTab('ai')} style={{ color: '#FFFFFF', background: activeTab === 'ai' ? '#121214' : 'transparent' }}>
              ...
              <div className={styles.navDivider} style={{ height: '1px', background: '#121214', margin: '1rem 0' }} />
      ...
      <main className={styles.main}>
        <header className={styles.dashboardHeader}>
          <div className={styles.headerTitle}>
            <h1 className={styles.title} style={{ color: isChef ? 'var(--primary)' : 'inherit' }}>{isChef ? 'MasterCart Delicacies' : 'MasterCart Marketplace'}</h1>
          ...
          <div className={styles.headerActions}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', background: isChef ? 'var(--primary-soft)' : 'var(--bg-200)', borderRadius: '20px', border: `1px solid ${isChef ? 'var(--primary)' : 'var(--border)'}` }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: isChef ? 'var(--primary)' : '#FFFFFF' }} />
          ...
          <div className={styles.tabContent}>
            ...
            {isTrialActive && (
              <div className={`${styles.escrowBanner} mb-4`} style={{ background: 'var(--grad-brand-soft)', borderColor: 'var(--primary)', marginBottom: '1.5rem', animation: 'fadeIn 0.5s ease' }}>
                ...
                <div className={styles.escrowText}>
                  ...
                  <p>You have UNLIMITED powers for the next <strong>{trialDaysLeft} days</strong>. Enjoy the full MasterCart experience!</p>
                </div>
              </div>
            )}
            ...
            <div className={styles.statsGrid}>
              ...
              <div className={`${styles.statCard} ${brand?.free_listings_count <= 2 ? styles.statCardWarning : ''}`}>
                ...
                <div className={styles.growthBadge} style={{ background: '#121214', color: '#FFFFFF' }}>Real-time</div>
              </div>
            </div>
              ...
              <div className={styles.settingsSection}>
                ...
                <div style={{ marginTop: '2.5rem' }}>
                  ...
                  <div className={styles.compactPricingGrid}>
                    {boostRates.map((boost) => (
                      <div key={boost.id} className={styles.compactPricingCard} style={boost.id === 'billboard' ? { border: '2px solid #27272A', background: '#121214' } : {}}>
                        <div className={styles.tierHeader}>
                ...
                <div className={styles.settingsSection}>
                  ...
                  <form onSubmit={handlePasswordChange} style={{ maxWidth: '400px' }}>
                    ...
                    {passSuccess && <div style={{ color: 'var(--success)', fontSize: '0.8rem', marginBottom: '1rem', background: '#121214', padding: '0.5rem', borderRadius: '4px' }}>Password updated successfully!</div>}
                    ...
                    <div className={styles.orderBody}>
                      ...
                      <div className={styles.statusCol}>
                        ...
                        {order.status === 'preorder_paid' && (
                          <div className={styles.statusBox}>
                            <div style={{ marginBottom: '0.5rem', padding: '0.4rem 0.75rem', background: '#121214', border: '1px solid #A0A0A0', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 600, color: '#FFFFFF', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                            ...
                            <button className="btn btn-primary btn-sm" style={{ width: '100%', background: 'var(--primary)', borderColor: 'var(--primary)', color: '#000000' }} onClick={() => updateOrderStatus(order.id, 'ready_for_pickup')}>
                          ...
                          return (
                            <label
                              ...
                              style={{
                                ...
                                background: isSelected ? '#121214' : 'var(--bg-200)',
          ...
          <div className={styles.tabContent}>
            <div className={styles.tabHeader}>
              <div>
                ...
                <p className={styles.subtitle}>Upload short videos of your latest delicacies to engage hungry students vividly.</p>
              </div>
              {!isAddingReel && (
            ...
            {isAddingReel ? (
              <div className={`card ${styles.formCard}`} style={{ maxWidth: '600px', margin: '0 auto 2rem auto' }}>
                ...
                <form onSubmit={handleReelSubmit}>
                  <div className="form-group">
                    ...
                    <div className={styles.mediaUploadArea} style={{ height: '200px' }}>
                      {newReel.video_url ? (
                        ...
                        <label className={styles.uploadPlaceholder} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', cursor: 'pointer' }}>
                  ...
                  <div className="form-group" style={{ marginBottom: '1rem' }}>
                    ...
                    <input 
                      type="text" 
                      className="form-input"
                      placeholder="e.g. Extra Spicy Jollof Combo" 
                      value={newReel.title}
                  ...
                  <div className="form-group" style={{ marginBottom: '1rem' }}>
                    ...
                    <textarea 
                      className="form-input"
                      placeholder="Tell hungry students about this delicacy..." 
                  ...
                  <div className="form-group" style={{ marginBottom: '1rem' }}>
                    ...
                    <div className={styles.productPicker} style={{ maxHeight: '150px', overflowY: 'auto', border: '1px solid var(--border)', borderRadius: '8px', padding: '0.5rem' }}>
                      {products.filter(p => !p.is_draft).map(product => (
                        <label key={product.id} className={styles.pickerItem} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.25rem 0' }}>
                          <input 
                            type="checkbox" 
          ...
          <div className={styles.tabContent}>
            ...
            <div className={styles.walletHero}>
              ...
              <div className={styles.walletEscrow} style={{ width: '100%', maxWidth: 'none', flexDirection: 'row', gap: '2rem', padding: '1.5rem', justifyContent: 'space-around' }}>
                ...
                <div style={{ width: '1px', background: '#121214' }}></div>
                <div>
          ...
          <div className={styles.tabContent}>
            ...
            <div className={styles.promoGrid}>
              ...
              <div className={styles.promoOption} style={{ 
                ...
                background: activeDelicaciesBillboard ? '#121214' : 'var(--bg-300)'
              }}>
                <div className={styles.promoIcon}><UtensilsCrossed size={24} color="#FFFFFF" /></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <h3 style={{ margin: 0 }}>Delicacies Billboard</h3>
                  <span className="badge badge-primary" style={{ background: '#121214' }}>PREMIUM</span>
                </div>
                ...
                {activeDelicaciesBillboard ? (
                  ...
                  <div style={{ marginTop: '1rem' }}>
                    ...
                    <button 
                      className="btn btn-primary btn-sm" 
                      style={{ background: 'var(--primary)', border: 'none', width: '100%', color: '#000000' }}
                     ...
                     {promoCodes.map(pc => (
                        <div key={pc.id} style={{ padding: '0.8rem', border: '1px solid var(--border)', borderRadius: '8px', marginBottom: '0.5rem' }}>
                           <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                              ...
                              <span style={{ color: '#10B981', fontWeight: 700 }}>{pc.type === 'percentage' ? `${pc.value}% off` : `₦${pc.value} off`}</span>
                           </div>
          ...
          <div style={{ padding: '2rem', maxWidth: 720, margin: '0 auto' }}>
            ...
            <div style={{ background: 'var(--bg-200)', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.25rem', border: '1px solid #27272A' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                >
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#121214', position: 'absolute', top: 3, transition: 'left 0.3s', left: aiSettings?.ai_enabled ? 26 : 4 }} />
                </button>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--bg-200)', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.25rem', border: '1px solid #27272A', opacity: aiSettings?.ai_enabled ? 1 : 0.5, pointerEvents: aiSettings?.ai_enabled ? 'auto' : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                >
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#121214', position: 'absolute', top: 3, transition: 'left 0.3s', left: aiSettings?.auto_reply_enabled ? 26 : 4 }} />
                </button>
              </div>
            </div>
      ...
      {showCopilot && (
        <div style={{
          ...
          {/* Header */}
          <div style={{ padding: '1rem 1.25rem', background: 'linear-gradient(135deg,rgba(255,255,255,0.2),rgba(0,0,0,0.1))', borderBottom: '1px solid #27272A', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {/* Input */}
          <form onSubmit={handleCopilotSubmit} style={{ padding: '0.75rem', borderTop: '1px solid #27272A', display: 'flex', gap: '0.5rem' }}>
            <input
              value={copilotInput}
      ...
      <button
        ...
        style={{
          ...
          border: 'none', cursor: 'pointer', display: 'none', alignItems: 'center', justifyContent: 'center',
