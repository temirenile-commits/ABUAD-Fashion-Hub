...
const BADGE_CONFIG: Record<string, { emoji: string; color: string; bg: string }> = {
  gold:   { emoji: '🥇', color: '#FFFFFF', bg: 'rgba(255,255,255,0.15)' },
  silver: { emoji: '🥈', color: '#FFFFFF', bg: 'rgba(255,255,255,0.15)' },
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <Link href="/delicacies" style={{
            ...
            color: '#FFFFFF', fontSize: '0.85rem', textDecoration: 'none', marginBottom: '1.5rem'
          }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: '2rem', fontWeight: 900, color: '#FFFFFF' }}>
      ...
      <div style={{ maxWidth: 800, margin: '2rem auto', padding: '0 1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '2rem', marginBottom: '2.5rem', borderBottom: '1px solid var(--border)' }}>
           <button 
             ...
             style={{ 
               background: 'none', border: 'none', color: activeTab === 'vendors' ? '#FFFFFF' : 'var(--text-400)', 
               fontWeight: 800, padding: '1rem 0', cursor: 'pointer', fontSize: '1rem',
               borderBottom: activeTab === 'vendors' ? '3px solid #FFFFFF' : '3px solid transparent',
               transition: 'all 0.2s'
             }}
           >
             👨‍🍳 Top Chefs
           ...
           <button 
             ...
             style={{ 
               background: 'none', border: 'none', color: activeTab === 'products' ? '#FFFFFF' : 'var(--text-400)', 
               fontWeight: 800, padding: '1rem 0', cursor: 'pointer', fontSize: '1rem',
               borderBottom: activeTab === 'products' ? '3px solid #FFFFFF' : '3px solid transparent',
               transition: 'all 0.2s'
             }}
           >
             🍲 Top Dishes
        ...
        {loading ? (
          ...
          <>
                    ...
                    <div key={r.id} style={{
                      ...
                      <div style={{ position: 'relative', width: 64, height: 64, margin: '0 auto 1rem' }}>
                        {brand?.logo_url ? (
                            ...
                            <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: cfg.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', fontWeight: 900, color: '#FFFFFF' }}>
                                {brand?.name?.[0] || '?'}
                            </div>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, fontSize: '0.8rem', color: '#FFFFFF', marginBottom: '0.5rem' }}>
                  ...
                  {top10.map((r, i) => {
                    ...
                    return (
                      <div key={r.id} style={{
                        ...
                        boxShadow: i < 3 ? '0 4px 12px rgba(255,255,255,0.05)' : 'none'
                      }}>
                        <div style={{ width: 32, textAlign: 'center', fontWeight: 900, color: i < 3 ? '#FFFFFF' : 'var(--text-400)', fontSize: '1.1rem' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', gap: '1rem', fontSize: '0.75rem', color: 'var(--text-400)', marginTop: 4 }}>
                            ...
                            {isProduct && <span style={{ color: '#FFFFFF', fontWeight: 800 }}>₦{r.price?.toLocaleString()}</span>}
                            {!isProduct && brand?.university_id === universityId && <span style={{ color: 'var(--primary)', fontVariant: 'small-caps' }}>Your Campus</span>}
                          </div>
                        </div>
                        {!isProduct && r.score && (
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontWeight: 900, fontSize: '1rem', color: '#FFFFFF' }}>{r.score.toFixed(0)}</div>
            ...
            <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
                    <UtensilsCrossed size={20} style={{ color: '#FFFFFF' }} />
                    ...
                    {allVendors.map(v => (
                        <Link key={v.id} href={`/vendor/${v.id}`} style={{
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.7rem', color: '#FFFFFF' }}>
