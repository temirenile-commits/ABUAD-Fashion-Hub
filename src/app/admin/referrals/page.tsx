...
const numericFields = [
  ...
  ['user_to_user_reward_purchase_limit', 'User → User qualifying purchases (0 = unlimited)'],
  ['vendor_referral_qualifying_sale_limit', 'User → Vendor qualifying sales (0 = unlimited)'],
] as const;
...
export default function AdminReferralPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.card}><div className={styles.cardHeader}><div><p className={styles.eyebrow}>Validated configuration</p><h2>Earning, expiry, and withdrawal rules</h2></div><button className="btn btn-primary" onClick={save} disabled={saving}><Save size={16} /> Save settings</button></div><div className={styles.formGrid}>{numericFields.map(([key,label]) => <label key={key}><span>{label}</span><input className="form-input" type="number" min="0" max={key.includes('percentage') ? '100' : undefined} value={config[key] ?? ''} onChange={e => setConfig(prev => ({ ...prev, [key]: e.target.value === '' ? null : Number(e.target.value) }))} /></label>)}</div></section>
    ...
    <section className={styles.card}><div className={styles.cardHeader}><div><p className={styles.eyebrow}>Existing payout architecture</p><h2>Payouts due and referral withdrawals</h2></div><ShieldCheck size={23} /></div><div className={styles.tableWrap}><table><thead><tr><th>User</th><th>Type</th><th>Verified account</th><th>Amount due</th><th>Status</th><th>Requested</th><th>Action</th></tr></thead><tbody>{(data?.payouts || []).map(row => <tr key={row.id}><td><strong>{row.user?.name || row.user?.email || row.user_id.slice(0,8)}</strong><small>{row.user_id.slice(0,8)}…</small></td><td>{row.source_metadata?.referral_type || 'Referral'}</td><td>{row.payout_account ? `${row.payout_account.bank_name} · ${row.payout_account.masked_account_number} · ${row.payout_account.verified_account_name}` : 'No verified account'}</td><td>₦{Number(row.amount_requested).toLocaleString()}</td><td>{row.status}</td><td>{new Date(row.created_at).toLocaleDateString()}</td><td>{['pending','processing','due'].includes(row.status) ? <span className={styles.actions}><button onClick={() => processPayout(row.id,'completed')}><CheckCircle2 size={15} /> Process</button><button onClick={() => processPayout(row.id,'rejected')}>Reject</button></span> : '—'}</td></tr>)}</tbody></table>{!data?.payouts?.length && <div className={styles.empty}>No referral withdrawals yet.</div>}</div></section>
  </main>;
}
