...
as $$
declare v_brand public.brands; v_requested public.universities; v_request public.vendor_university_change_requests; v_admin record; v_reason text := trim(coalesce(p_reason, ''));
begin
  ...
  if coalesce(v_brand.verified,false) is not true or lower(coalesce(v_brand.verification_status,'')) not in ('approved','verified') then raise exception 'University target changes are available only to verified vendors'; end if;
  ...
  if exists(select 1 from public.vendor_university_change_requests where vendor_id=v_brand.id and requested_university_id=p_requested_university_id and status='PENDING') then raise exception 'A request for this target university is already pending'; end if;
  ...
  values(v_brand.id,p_requesting_user_id,v_brand.university_id,p_requested_university_id,v_reason,coalesce(v_brand.verification_status,'unknown')) returning * into v_request;
  for v_admin in select id from public.users where role in ('university_admin','university_staff','admin','super_admin') and (role in ('admin','super_admin') or university_id=p_requested_university_id) loop
    insert into public.notifications(user_id,type,title,content,link,is_read,university_id) values(v_admin.id,'vendor_university_change','New vendor university change request','A verified vendor has requested to target your university marketplace.','/university-admin?tab=university_change_requests',false,p_requested_university_id);
  end loop;
  return v_request;
end;
$$;

create or replace function public.review_vendor_university_change_request(p_request_id uuid,p_admin_id uuid,p_decision text,p_message text default null)
returns public.vendor_university_change_requests language plpgsql security definer set search_path = public
as $$
declare v_request public.vendor_university_change_requests; v_brand public.brands; v_admin public.users; v_target public.universities; v_title text; v_content text;
begin
  ...
  if upper(p_decision)='APPROVE' then
    ...
    v_request.status:='APPROVED'; v_title:='University target change approved'; v_content:='Your request to change your target university has been approved.';
  elsif upper(p_decision)='REJECT' then v_request.status:='REJECTED'; v_title:='University target change rejected'; v_content:='Your request to change your target university was not approved.';
  ...
  insert into public.notifications(user_id,type,title,content,link,is_read,university_id) values((select owner_id from public.brands where id=v_request.vendor_id),'vendor_university_change',v_title,v_content||case when nullif(trim(coalesce(p_message,'')),'') is not null then ' Admin message: '||trim(p_message) else '' end,'/dashboard/vendor?tab=settings',false,v_request.requested_university_id);
  return v_request;
end;
$$;
